...
Item {
  ...
  function open(payloadJson) {
    ...
    // The icon index can lag the shell's start (first-install packages, slow
    // scans); ask for a rescan on open so app and window icons appear live.
  ...
  // Third-party menu clones should receive a scoped AppLibrary proxy from the
  // host. Keep a local instance as a compatibility fallback for shell versions
  // where that capability is not populated.
  AppLibrary {
    id: localAppLibrary
  }
  ...
  function rebuildItemsFromSources() {
    // The Windows submenu is native to this clone, so it is spliced into the
    // defaults right after Apps instead of living in a JSONC file — the merge
    // keeps file order, and the user extension file stays revertible.
    var defaults = (root.defaultMenuItems || []).slice()
    var windowsItem = root.normalizeItem("windows", {
      icon: "󰖯",
      label: "Windows",
      aliases: ["windows", "switch window", "running"],
      description: "Open windows",
      provider: "windows"
    })
    var appsAt = -1
    for (var i = 0; i < defaults.length; i++) {
      if (defaults[i] && defaults[i].id === "apps") { appsAt = i; break }
    }
    if (appsAt >= 0) defaults.splice(appsAt + 1, 0, windowsItem)
    else defaults.push(windowsItem)
  ...
  readonly property var providers: ({
    ...
    // QML-native like "apps": rows come from the live Hyprland toplevel model
    // (mergeWindowRows below), never from a bash enumeration, so the entry
    // carries no script. Volatile so re-entering the submenu refreshes it.
    "windows": {
      native: true,
      volatile: true
    }
  ...
  // ------------------------------------------------------------------
  // Running windows. Live rows from the Hyprland IPC model — the same
  // reactive source the Workspaces bar widget watches — swapped like any
  // provider batch so closed windows drop out on the next merge.
  // ------------------------------------------------------------------
  ...
  // Resolve the compositor class to its desktop entry before asking the icon
  // theme. Window classes are not icon names (for example, class "wechat"
  // uses Icon=wechat-universal), while StartupWMClass is made for this match.
  function windowIconName(cls, initialClass, appId) {
    if (!root.appLibrary) return String(cls || "").toLowerCase()

    var windowKeys = [cls, initialClass, appId].map(root.normalizedAppKey)
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      var startupClass = root.normalizedAppKey(entry.startupClass)
      var desktopId = root.normalizedAppKey(entry.id)
      var name = root.normalizedAppKey(entry.name)
      var score = -1
      if (startupClass && windowKeys.indexOf(startupClass) >= 0) score = 300
      else if (desktopId && windowKeys.indexOf(desktopId) >= 0) score = 200
      else if (name && windowKeys.indexOf(name) >= 0) score = 100
      if (score > bestScore && String(entry.icon || "")) {
        bestScore = score
        bestIcon = String(entry.icon)
      }
    }
    return bestIcon || String(cls || appId || "").toLowerCase()
  }

  function windowRowFor(toplevel) {
    var ipc = toplevel.lastIpcObject || {}
    var appId = String(toplevel.wayland ? toplevel.wayland.appId : "")
    var cls = String(ipc["class"] || appId || "?")
    var initialClass = String(ipc["initialClass"] || "")
    var title = String(toplevel.title || ipc["title"] || cls)
    // The IPC address can arrive without its 0x prefix; the dispatch needs it.
    var address = String(toplevel.address || "")
    if (address && address.indexOf("0x") !== 0) address = "0x" + address
    var ws = toplevel.workspace
    var wsId = ws ? ws.id : 0
    var wsLabel = ws ? (ws.id < 0 ? String(ws.name).replace("special:", "") : "WS " + ws.id) : ""
    var monitor = toplevel.monitor
    var detail = "Running · " + (wsLabel || "?") + (monitor ? " · " + monitor.name : "")
    return {
      id: "windows." + address,
      parent: "windows",
      kind: "window",
      icon: "",
      appIcon: root.windowIconName(cls, initialClass, appId),
      appId: "",
      label: title,
      title: "",
      target: "",
      description: detail,
      // hyprlua Hyprland parses dispatch IPC as Lua — hl.dsp.focus is the
      // form omarchy-hyprland-focus-app uses. Runs via execDetached after
      // the menu has closed, same as the launch-or-focus actions.
      action: "hyprctl dispatch \"hl.dsp.focus({ window = 'address:" + address + "' })\"",
      provider: "",
      aliases: [cls],
      when: "",
      checked: "",
  ...
  function mergeWindowRows() {
    var rows = []
    var toplevels = root.toplevels || []
    for (var i = 0; i < toplevels.length; i++) {
      var toplevel = toplevels[i]
      var ipc = toplevel.lastIpcObject || {}
      // Unmapped toplevels are closing or not yet on screen.
      if (ipc["mapped"] === false) continue
      rows.push(root.windowRowFor(toplevel))
    }
    // Active workspace first, then workspace id, then title.
    var focusedWsId = Hyprland.focusedWorkspace ? Hyprland.focusedWorkspace.id : 0
    rows.sort(function(a, b) {
      var wa = a.wsId === focusedWsId ? -1e9 : a.wsId
      var wb = b.wsId === focusedWsId ? -1e9 : b.wsId
      if (wa !== wb) return wa - wb
      return a.label.localeCompare(b.label)
    })

    var merged = MenuModel.swapProviderRows(root.items, root.itemOrder, "windows", rows)
    root.items = merged.items
    root.itemOrder = merged.itemOrder
    if (root.opened) root.rebuildDisplay()
  }

  // Titles, focus and the window set itself move while the menu is open.
  Connections {
    target: Hyprland

    function onRawEvent(event) {
      if (!root.opened) return
      var refreshOn = ["openwindow", "closewindow", "movewindow", "windowtitle",
        "activewindow", "activewindowv2", "workspace", "changefloatingmode"]
      if (refreshOn.indexOf(event.name) >= 0 && root.providersLoaded["windows"]) root.mergeWindowRows()
    }
  }
  ...
  function startProviderForMenu(id) {
    ...
    if (entry.provider === "apps" || entry.provider === "windows") {
      root.providersLoaded[id] = true
      if (entry.provider === "apps") root.mergeAppRows()
      else root.mergeWindowRows()
      return
    }
  ...
  function rebuildDisplay() {
    ...
    if (query) {
      ...
      // Running windows are the most immediately actionable hits: lift every
      // matching window row above the rest, app rows included. Their section
      // is cleared so the drilldown divider does not render twice around the
      // hoisted block.
      var windowRows = []
      var otherRows = []
      for (var w = 0; w < rows.length; w++) {
        if (rows[w].kind === "window") {
          rows[w].section = ""
          windowRows.push(rows[w])
        } else {
          otherRows.push(rows[w])
        }
      }
      if (windowRows.length > 0) rows = windowRows.concat(otherRows)
    } else {
        ...
        Item {
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              readonly property bool isApp: row.kind === "app" || row.kind === "window"
              ...
              Column {
                ...
                Text {
                  ...
                  width: parent.width
                  text: row.detail
                  visible: (root.filterText || row.kind === "dmenu" || row.kind === "window") && row.detail.length > 0
                  color: root.foreground
                  opacity: 0.52
                  font.family: root.fontFamily
